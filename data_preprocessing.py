...
RASTER_FILES = Path("~/finalproject475-main/Raster_Burn_Data")
PREPROCESSED_FILES = Path("/processed_to_npy")
CHIP_SIZE = 256
STRIDE = 128
...
def process_file(filename):
    ...
    split = get_filenames(filename)  # determine the split
    save_dir = PREPROCESSED_FILES / split
    save_dir.mkdir(parents=True, exist_ok=True)

    data = padding_rasterfiles(file_path, CHIP_SIZE)  # calculate padding

    data = normalize_raster(data)  # normalizing the rasters

    chips = extract_chips(data, CHIP_SIZE, STRIDE)  # extract chips

    num_chips = save_chips(chips, filename, save_dir)  # saving chips
